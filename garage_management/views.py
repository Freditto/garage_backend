...
@api_view(['GET'])
@permission_classes([AllowAny])
def GetEngineer(request, garage_id):
    garage = Garage.objects.get(id=garage_id)
    engineer = Engineer.objects.values('id', 'username', 'phone', 'description').filter(garage_id=garage)
...
@api_view(['GET'])
@permission_classes([AllowAny])
def GetNearByGarage(request):
    long = request.GET.get('long', None)
    lat = request.GET.get('lat', None)
    garages = Garage.objects.all()
    # print(garages.latitude+garages.longitude)
    nearby_garages = []
...
@api_view(['GET'])
@permission_classes([AllowAny])
def GarageAcceptedFeeds(request, driver_id):
    driver = User.objects.get(id=driver_id)
    print(driver_id)
    feed_requests = FeedBackRequest.objects.values('id', 'driver_id', 'latitude', 'longitude', 'is_received').filter(driver_id=driver)
    print(feed_requests)
    fr = [e for e in feed_requests]
    feedback_request = []
    for d in fr:
    ...
    feed_appointment = FeedBackAppointment.objects.values('id', 'driver_id', 'date', 'created_at', 'is_received').filter(driver_id=driver)
    fa = [e for e in feed_appointment]
    feedback_appointment = []
    for d in fa:
        user = User.objects.get(id=d['driver_id'])
